...
####################################################################
#### 1) Load data and model fits from previous analyses         ####
load("../Additional_Analyses/collected_fitsNpredicts_KonfMaskTime_review.RData")
Preds_MRating_corr_cond <- mutate(Preds_MRating_corr_cond, 
...
#                                            "dynamical visibility", "drift diffusion RT")
...
p_Acc <- ggplot(Data_Acc,
  ...
  #scale_y_continuous(limits = c(1, 5), breaks=1:5, labels = c("0", "25", "50", "75", "100"), name="Mean Accuracy")+
  theme_bw() +
...
Preds_RTQuants_corr_rating <- Preds_RTQuants_corr_rating %>%
  mutate(X = factor(paste(rating, correct, sep="."),
                    levels = c(paste(c(5:1, 1:5), rep(0:1, each=5), sep=".")),
                    labels = c(paste(c(5:1, 1:5), rep(0:1, each=5), sep="."))))
# Preds_RTQuants_corr_rating$model <- factor(Preds_RTQuants_corr_rating$model, 
...
ggplot()+
  # geom_point( data=mutate(Preds_RTQuants_corr_rating, correct=factor(correct, labels=c("Wrong", "Correct")),
  #                         rating = as.factor(rating)),
  #             aes(x=rating, y=log(q),shape="Model fit"),
  #             size=1.5, fill="white", color="white") +
  # geom_line(data=mutate(Data_RTQuants_corr_rating,, correct=factor(correct, labels=c("Wrong", "Correct")),
  #                       rating = as.factor(rating)),
  #           aes(x=rating, y=log(q), group=as.factor(p), linetype="Observed", color="Observed"), size=0.7)+
...
table_BIC_comparison <- xtable(print_BIC_table, align = c("l", "l", "c", "c", "c"),
                        caption="\\raggedright Results from quantitative mode comparison with dynWEV using BIC for experiment 1")
addtorow <- list()
...
print(table_BIC_comparison, type="latex", file="../../Supplement/figures/tableBICs1.tex",sanitize.text.function=function(x){x},
      include.rownames=FALSE,
      add.to.row=addtorow,
      hline.after = c(0, nrow(table_BIC_comparison)), booktabs = TRUE,
      caption.placement="top",
...
ggplot()+
  ...
  scale_y_continuous(breaks = log(c(1.5, 2.5, 4, 6)),
                     # labels = paste("log(", c(1.5, 2.5, 4, 7), ")", sep=""), 
                     labels = paste(c(1.5, 2.5, 4, 6), sep=""), 
                     name="Reaction time quantiles [s] (log scaled)")+
...
ggsave("figures/RTQuantsCond1.eps",
       width=17.62, height=22, dpi=1200, units="cm", device=cairo_ps)
