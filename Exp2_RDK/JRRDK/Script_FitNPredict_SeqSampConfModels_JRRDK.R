...
Data <- read.table("dataJRRDK.csv", header=TRUE, sep=",")
Data <- group_by(Data, participant)
Ntotal <-  Data %>%
  summarise(Ntot = sum(n())) 
Data <- left_join(Data, Ntotal)
...
BadSubjects <- CheckParticipants %>%
  filter(nTrials < 640 | Performance < .5 | 
           AboveChance < 3 | NumModeRating > .9)
...
cat(paste("Participants that quit the experiment early:", paste((subset(BadSubjects, nTrials < 640))$participant, collapse = ", "), "\n"))
cat(paste("Excluded Participants based on Accuracy:", paste((subset(BadSubjects, nTrials > 640))$participant, collapse = ", "), "\n"))
...
preds_WEV <- ddply(fits_WEVmodels,.(participant, model),
                    function(df) predictWEV_Conf(df, model = df$model[1], subdivisions = 1000, maxrt = maxrt,
                                                 simult_conf = TRUE),
                    .parallel = T)
...
Preds_RTdens_corr_cond_rating <-  rbind(RT_dist_RM, RT_dist_WEV) %>% 
  left_join(Ns_part) %>%
  ungroup() %>%
  select(-participant) %>%
